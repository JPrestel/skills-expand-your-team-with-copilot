...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty ('beginner', 'intermediate', 'advanced', 'all-levels')
    """
    ...
    if difficulty:
        ...
        if normalized_difficulty == "all-levels":
            query["difficulty"] = {"$exists": False}
        elif normalized_difficulty in difficulty_levels:
            query["$or"] = [
                {"difficulty": difficulty_levels[normalized_difficulty]},
                {"difficulty": {"$exists": False}}
            ]
        else:
            raise HTTPException(
                status_code=400,
                detail="Difficulty must be beginner, intermediate, advanced, or all-levels"
            )
